...
// @updateURL https://github.com/olivierdagenais/tampermonkey-copy-url/raw/release/2.20/userscript/index.user.js
// @downloadURL https://github.com/olivierdagenais/tampermonkey-copy-url/raw/release/2.20/userscript/index.user.js
// @match *://*/*
...
const Html_1 = __webpack_require__(18);
const JenkinsBuild_1 = __webpack_require__(19);
const JenkinsConfigure_1 = __webpack_require__(20);
const JenkinsConsole_1 = __webpack_require__(22);
const JenkinsCredentials_1 = __webpack_require__(23);
const JenkinsDashboard_1 = __webpack_require__(24);
const JiraLinkIssue_1 = __webpack_require__(25);
const JiraSave_1 = __webpack_require__(26);
const JenkinsNext_1 = __webpack_require__(27);
const JenkinsPipelineSyntax_1 = __webpack_require__(28);
const JenkinsPrevious_1 = __webpack_require__(29);
const JenkinsUp_1 = __webpack_require__(30);
const JiraWorklog_1 = __webpack_require__(31);
const KeyboardShortcut_1 = __webpack_require__(32);
const Markdown_1 = __webpack_require__(33);
const Textile_1 = __webpack_require__(34);
...
const Jira_1 = __webpack_require__(14);
const ServiceDesk_1 = __webpack_require__(15);
const ServiceNow_1 = __webpack_require__(16);
const Zabbix_1 = __webpack_require__(17);
...
class Jenkins extends AbstractParser_1.AbstractParser {
    parseLink(doc, url) {
        ...
        const jenkinsPage = JenkinsHelpers_1.JenkinsHelpers.parsePage(bodyElement, url);
        ...
        let linkText = ``;
        jenkinsPage.eachCrumb((crumb, key) => {
            var _a;
            if (key > 0) {
                ...
                linkText += (_a = crumb.name) === null || _a === void 0 ? void 0 : _a.trim();
...
const Crumb_1 = __webpack_require__(12);
const JenkinsPage_1 = __webpack_require__(13);
const jenkins2_463 = SemVer_1.SemVer.parse("2.463");
const DashboardCrumb = new Crumb_1.Crumb("Dashboard", "/");
class JenkinsHelpers {
    ...
    static parsePage(bodyElement, urlString) {
        ...
        if (anchorContainers) {
            ...
            for (let i = 0; i < anchorContainers.length; i++) {
                ...
                if (anchor) {
                    ...
                    if (path) {
                        const crumb1 = new Crumb_1.Crumb(anchorContainer.textContent, path);
                        crumbs.push(crumb1);
                    }
                    ...
                    if (i == anchorContainers.length - 1) {
                        const url = new URL(urlString);
                        const crumb = new Crumb_1.Crumb(anchorContainer.textContent, url.pathname);
                        crumbs.push(crumb);
                    }
                }
            }
            return new JenkinsPage_1.JenkinsPage(urlString, crumbs, bodyElement);
        }
        return new JenkinsPage_1.JenkinsPage(urlString, [DashboardCrumb], bodyElement);
    }
...
const JenkinsHelpers_1 = __webpack_require__(10);
class JenkinsPage {
    constructor(urlString, crumbs, bodyElement) {
        this.urlString = urlString;
        this.urlParts = JenkinsHelpers_1.JenkinsHelpers.splitUrlPath(urlString);
        this.crumbs = crumbs;
        this.bodyElement = bodyElement;
    }
    buildUrl(path, queryString) {
        return JenkinsHelpers_1.JenkinsHelpers.buildUrl(this.urlString, path, queryString);
    }
    eachCrumb(callback) {
    ...
    findJobCrumb() {
        let result = null;
        this.reverseCrumb((crumb, key) => {
            const urlParts = JenkinsHelpers_1.JenkinsHelpers.splitPath(crumb.href);
            const numParts = urlParts.length;
            if (numParts >= 2) {
                const isUrlToJob = "job" === urlParts[numParts - 2];
                if (isUrlToJob) {
                    result = crumb;
                    return false;
                }
...
class JenkinsBuild {
    ...
    navigate(doc, url) {
        ...
        if (bodyElement.querySelector(linkSelector)) {
            ...
            const destinationUrl = jenkinsPage.buildUrl(crumb.href + "build", "delay=0sec");
            return new Request(destinationUrl, PostOptions);
        }
        else {
            let result = null;
            jenkinsPage.reverseCrumb((crumb) => {
                const hrefParts = JenkinsHelpers_1.JenkinsHelpers.splitPath(crumb.href);
                if ("job" == hrefParts[hrefParts.length - 2]) {
                    const destinationUrl = JenkinsHelpers_1.JenkinsHelpers.buildUrl(url, crumb.href + "build", "delay=0sec");
...
const GoToAction_1 = __webpack_require__(21);
const JenkinsHelpers_1 = __webpack_require__(10);
class JenkinsConfigure extends GoToAction_1.GoToAction {
    navigate(doc, urlString) {
        ...
        const jenkinsPage = JenkinsHelpers_1.JenkinsHelpers.parsePage(bodyElement, urlString);
...
class JenkinsCredentials extends GoToAction_1.GoToAction {
    navigate(doc, url) {
        ...
        return jenkinsPage.pageOrRootAction(action);
    }
}
...
class JenkinsDashboard extends GoToAction_1.GoToAction {
    navigate(doc, urlString) {
        ...
        return jenkinsPage.buildUrl(firstCrumb.href);
    }
}
...
class JenkinsPrevious extends GoToAction_1.GoToAction {
    navigate(doc, urlString) {
        ...
        const jenkinsPage = JenkinsHelpers_1.JenkinsHelpers.parsePage(bodyElement, urlString);
        var urlParts = jenkinsPage.urlParts;
